...
public class Generator {
    ...
    public void triggerRedAlert() {
        Location spawnLoc = location.clone().add(0.5, 1.0, 0.5);
        ItemDisplay redAura = location.getWorld().spawn(spawnLoc, ItemDisplay.class, display -> {
            display.setTransformation(new Transformation(
                    new Vector3f(0f, 0f, 0f),
                    new AxisAngle4f(0f, 0f, 0f, 1f),
                    new Vector3f(2.05f, 2.05f, 2.05f),
                    new AxisAngle4f(0f, 0f, 0f, 1f)));
            display.setItemStack(new ItemStack(Material.REDSTONE_BLOCK));
            display.setViewRange(50f);
        ...
        for (Player p : Bukkit.getOnlinePlayers()) {
            boolean isKiller = DBDPlugin.getInstance().getGameManager() != null
                    && DBDPlugin.getInstance().getGameManager().isKiller(p);
            if (isKiller) {
                p.showEntity(DBDPlugin.getInstance(), redAura);
            }
        }
        Bukkit.getScheduler().runTaskLater(DBDPlugin.getInstance(), () -> {
            if (redAura.isValid()) {
                redAura.remove();
            }
        }, 60L);
