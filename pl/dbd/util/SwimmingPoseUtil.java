...
public class SwimmingPoseUtil {
   ...
   private static void forceViaNMS(Player player) throws Exception {
      ...
      Class entityClass;
      for (entityClass = nmsPlayer.getClass().getSuperclass(); entityClass != null
            && !entityClass.getSimpleName().equals("Entity"); entityClass = entityClass.getSuperclass()) {
      }
      ...
      if (entityClass == null) {
         ...
         for (int var6 = 0; var6 < var5; ++var6) {
            Field f = var4[var6];
            if (f.getType().getSimpleName().contains("SynchedEntityData")
                  || f.getType().getSimpleName().contains("EntityDataManager")
                  || f.getType().getSimpleName().contains("DataWatcher")) {
               dataField = f;
               break;
            }
         }
         ...
         if (dataField == null) {
            ...
            for (int var8 = 0; var8 < var15; ++var8) {
               Field f = var13[var8];
               if (f.getType().getSimpleName().contains("EntityDataAccessor")
                     || f.getType().getSimpleName().contains("DataWatcherObject")) {
            ...
            if (flagsAccessorField == null) {
               ...
               Method setMethod = synchedData.getClass().getMethod("set", flagsAccessor.getClass().getInterfaces()[0],
                     Object.class);
