...
public class UniversalProtectionListener implements Listener {
   ...
   public void onCommandPreprocess(org.bukkit.event.player.PlayerCommandPreprocessEvent event) {
      ...
      if (gm.getReadyPlayers().contains(p.getUniqueId()) || gm.getQueuedPlayers().contains(p.getUniqueId())) {
         String message = event.getMessage().toLowerCase();
         String[] args = message.split(" ");
         String command = args[0];

         java.util.List<String> blockedCommands = plugin.getConfig().getStringList("blocked-lobby-commands");
