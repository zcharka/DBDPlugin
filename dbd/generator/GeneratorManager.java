...
public class GeneratorManager {
    ...
    public void save() {
        ...
        for (Map.Entry<String, List<Generator>> entry : mapGenerators.entrySet()) {
            ...
            for (int i = 0; i < gens.size(); i++) {
                ...
                data.set(path, loc.getWorld().getName() + "," + loc.getBlockX() + "," + loc.getBlockY() + ","
                        + loc.getBlockZ());
            }
        }
        try {
    ...
    public void updateAurasForPlayer(Player p, boolean isKiller) {
        for (Generator gen : activeGenerators) {
            gen.updateAuraVisibility(p, isKiller);
        }
    }
    ...
    public void createGenerator(Player player, String mapName) {
        ...
        for (List<Generator> gens : mapGenerators.values()) {
            for (Generator g : gens) {
                if (g.getLocation().getBlock().equals(loc.getBlock())) {
                    player.sendMessage(pl.dbd.DBDPlugin.getMsg("ctujujestgeneratormo"));
                    return;
                }
            }
        }

        Generator newGen = new Generator(loc, player);
        ...
        if (mapName.equalsIgnoreCase(currentMapName)) {
            ...
            player.sendMessage(pl.dbd.DBDPlugin.getMsg("7niewidagoterazbozaa"));
        }

        save();
    }

    public boolean removeClosestGenerator(Location loc, double radius) {
        boolean removedAny = false;
        double minDistance = radius * radius;
        Generator closest = null;
        List<Generator> closestList = null;

        for (List<Generator> gens : mapGenerators.values()) {
            for (Generator gen : gens) {
                if (gen.getLocation().getWorld().equals(loc.getWorld())) {
                    double dist = gen.getLocation().distanceSquared(loc);
                    if (dist < minDistance) {
                        minDistance = dist;
                        closest = gen;
                        closestList = gens;
        ...
        if (closest != null) {
            closestList.remove(closest);
            activeGenerators.remove(closest);
            removedAny = true;
        }
    ...
    public int getCompletedCount() {
        return (int) activeGenerators.stream().filter(Generator::isCompleted).count();
    }
