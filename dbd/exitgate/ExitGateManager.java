...
public class ExitGateManager {
   ...
   public void triggerGatesAura() {
      for (ExitGate gate : activeGates) {
         Location spawnLoc = gate.getLocation().clone().add(0.5, 1.0, 0.5);
         org.bukkit.entity.ItemDisplay glowAura = spawnLoc.getWorld().spawn(spawnLoc,
               org.bukkit.entity.ItemDisplay.class, display -> {
                  display.setTransformation(new org.bukkit.util.Transformation(
                        ...
                        new org.joml.Vector3f(4.5f, 4.5f, 4.5f),
                        new org.joml.AxisAngle4f(0f, 0f, 0f, 1f)));
                  display.setItemStack(new org.bukkit.inventory.ItemStack(org.bukkit.Material.GLOWSTONE));
                  display.setViewRange(50f);
                  display.setVisibleByDefault(false);
                  display.setBillboard(org.bukkit.entity.ItemDisplay.Billboard.FIXED);
                  display.setGlowing(true);
                  try {
                     display.setGlowColorOverride(org.bukkit.Color.ORANGE);
                  } catch (Throwable ignored) {
                  }
               });

         for (Player p : Bukkit.getOnlinePlayers()) {
            boolean isSurvivor = plugin.getGameManager() != null
                  && plugin.getGameManager().isSurvivor(p);
            if (isSurvivor) {
               p.showEntity(plugin, glowAura);
            }
         }
