...
public class EquipmentManager {
    ...
    // --- KILLER LOGIC (SINGLETON EQUIVALENT TO ITEMS) ---
    public List<EqEntry> getOwnedKillers(Player p) {
        List<EqEntry> out = new ArrayList<>();
        ConfigurationSection inv = data.getConfigurationSection(uuid(p) + ".inventory");
        if (inv != null) {
            for (String id : inv.getKeys(false)) {
                if (inv.getInt(id) > 0) {
                    EqEntry e = registry.get(id);
                    if (e != null && e.role() == EqEntry.Role.KILLER && e.type() == EqEntry.Type.KILLER)
                        out.add(e);
                }
            }
        }
    ...
    public String getEquippedKiller(Player p) {
        return data.getString(uuid(p) + ".equipped.KILLER.killer", null);
    }

    public void toggleEquippedKiller(Player p, String id) {
        String current = getEquippedKiller(p);
        if (id.equals(current)) {
            data.set(uuid(p) + ".equipped.KILLER.killer", null);
        } else {
            data.set(uuid(p) + ".equipped.KILLER.killer", id);
        }
        save();
    }
    ...
    public void giveMatchEquipment(Player p, EqEntry.Role role) {
        ...
        // Jeżeli rola to KILLER podaj też wyposażonego Killera
        if (role == EqEntry.Role.KILLER) {
            String killerId = getEquippedKiller(p);
            if (killerId != null && registry.containsKey(killerId)) {
                EqEntry kEntry = registry.get(killerId);
                p.getInventory().addItem(createEqItemStack(kEntry));
            }
        }
    }
    ...
    public void toggleGuiRole(String id) {
        ...
        String nextRole = cur.equalsIgnoreCase("SURVIVOR") ? "KILLER" : "SURVIVOR";
        data.set(path, nextRole);
        
        if (nextRole.equals("KILLER")) {
            String typePath = "equipment." + id + ".type";
            if (data.getString(typePath, "ITEM").equalsIgnoreCase("ITEM")) {
                data.set(typePath, "PERK");
            }
        } else {
            String typePath = "equipment." + id + ".type";
            if (data.getString(typePath, "ITEM").equalsIgnoreCase("KILLER")) {
                data.set(typePath, "PERK");
            }
        }
    ...
    public void toggleGuiType(String id) {
        ...
        String rolePath = "equipment." + id + ".role";
        String cur = data.getString(path, "ITEM");
        String role = data.getString(rolePath, "SURVIVOR");
        String next;
        
        if (role.equalsIgnoreCase("KILLER")) {
            // Killer: PERK -> KILLER -> PERK
            if (cur.equalsIgnoreCase("PERK")) next = "KILLER";
            else next = "PERK";
        } else {
            // Survivor: ITEM -> PERK -> ITEM
            if (cur.equalsIgnoreCase("ITEM")) next = "PERK";
            else next = "ITEM";
        }
        data.set(path, next);
        save();
